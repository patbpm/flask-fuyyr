...
def venues():
  ...
  #       num_upcoming_shows should be aggregated based on number of upcoming shows per venue.
    ...
    for location in locations:
        ...
        venue = []
        for t in location_venues:
            venue.append({
                'id': t.id,
                'name': t.name,
                'num_upcoming_shows': len(
                    db.session.query(Show).filter(Show.venue_id == t.id).filter(Show.start_time >                                          datetime.now()).all())
            })
  ...
  # search for "Music" should return "The Musical Hop" and "Park Square Live Music & Coffee"
    ...
    for resp in search_results:
        data.append({
            ...
            'name': resp.name,
            'num_upcoming_shows':
                len(db.session.query(Show).filter(Show.venue_id == resp.id).
...
def delete_venue(venue_id):
  ...
  # SQLAlchemy ORM to delete a record. Handle cases where the session commit could fai
    error = False
    try:
        venue = Venue.query.get(venue_id)
